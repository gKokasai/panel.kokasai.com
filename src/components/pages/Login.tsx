    ...
    return (
      <PasswordRequestForm
        ...
        onClickForwardButton={() =>
          auth.setRequest({ ...auth.request, isRequestPassword: true })
        }
  ...
  return (
    <LoginForm
      ...
      onChangePassword={(event) =>
        auth.setRequest({ ...auth.request, inputPassWord: event.target.value })
      }
      id={auth.request.inputId}
      onClickBackButton={() =>
        auth.setRequest({ ...auth.request, isRequestPassword: false })
      }
    />
  );
};
