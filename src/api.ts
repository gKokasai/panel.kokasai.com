 ...
 * @param pass パスワード
 */
export const postAuth = (id?: string, pass?: string) => {
  const header:{
 ...
 * @param id ログインする学籍番号
 */
export const postLogin = (id?: string) => axios.post(`${URL}/login`, { id });

/**
 * ログアウトする。
 * [POST /logout](https://github.com/gKokasai/api.kokasai.com/blob/master/DOCUMENT.md#post-logout)
 */
export const postLogout = () => axios.post(`${URL}/logout`);
...
export const postGroupDocumentList = (groupName: string, list: {
  document: string[]
}) => axios.post(`${URL}/group/document/list/${groupName}`, list);
 ...
 * @param groupName 取得するグループの名前
 */
export const getGroupUserList = (groupName: string) => axios.get(`${URL}/group/user/list/${groupName}`);
...
export const postGroupUserList = (groupName: string, list: {
  owner: string[],
  member: string[]
}) => axios.post(`${URL}/group/user/list/${groupName}`, list);
 ...
 */
export const getUserDocumentList = () => axios.get(`${URL}/user/document/list`);
 ...
 */
export const getUserGroupList = () => axios.get(`${URL}/user/group/list`)
